 ...
 *
 * After the layout engine places nodes into columns,
 * renders faint lane backgrounds + column headers per phase.
 */
...
export default function ColumnLabels() {
  ...
  useEffect(() => {
    ...
    // Derive depth from dependency graph
    ...
    const entries: ColumnInfo[] = sortedCols.map(([col, labels]) => ({
      label: col === 0 ? 'Foundation'
        : col === sortedCols.length - 1 ? 'Integration'
        : `Phase ${col + 1}`,
      col,
